...
describe('playIcons', () => {
    ...
    test('defensive 2-pt, late-down conversions, long returns', () => {
        ...
        // just under the shared +1.1 cutoff, on both sides of the sign flip
        expect(playIcons(play({ kickoff_play: true, EPA: 1.05 } as any, 'Kickoff Return (Offense)'))).toEqual([]);
        expect(playIcons(play({ punt_play: true, EPA: -1.05 } as any, 'Punt Return'))).toEqual([]);
        expect(playIcons(play({ kickoff_play: true, EPA: 1.1 } as any, 'Kickoff Return (Offense)'))).toEqual(['explosive']);
        expect(playIcons(play({ punt_play: true, EPA: -1.1 } as any, 'Punt Return'))).toEqual(['explosive']);
