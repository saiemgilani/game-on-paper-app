...
// `header` (not `gameInfo`), `advBoxScore` (not `boxScore`), per-play `winProbability`,
// top-level `homeTeamSpread`/`overUnder` (there is no `pickcenter` on ProcessedGame),
// `broadcasts` (there is no `leaders` on ProcessedGame), `drives.previous`.
const fullGame = () => ({
...
describe('evaluateManifest', () => {
  ...
  test('missing optional -> degraded', () => {
    ...
    expect(v.missingNames).toContain('odds');
